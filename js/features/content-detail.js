...
(function initializeContentDetail(window) {
  ...
  function render() {
    ...
    const bufferDrafts = content.buffer?.state === 'posts';
    el('contentDetailMeta').innerHTML = `<span class="content-status status-${lifecycle}">${label(lifecycle)}</span>${content.targetDate ? `<span>Target ${safeText(content.targetDate)}</span>` : ''}${content.pillarId ? `<span>${safeText(pillarName(content.pillarId) || 'Content pillar')}</span>` : ''}<span>${content.buffer?.contentItemId ? 'Draft saved to Buffer' : 'Local draft'}</span>${itemCount ? `<span>${itemCount} ${bufferDrafts ? 'Buffer' : 'channel'} draft${itemCount === 1 ? '' : 's'}</span>` : ''}`;
    el('contentDetailSync').textContent = content.buffer?.contentItemId ? 'Update Buffer Draft' : 'Save Draft to Buffer';
